...
-   多重背包
    ...
    -   状态转移方程
        ...
        $dp[i][j] = \max(dp[i - 1][j - kv_i] + kw_i)$

    -   优化

        -   将物品数分解为 $2^i$ ，则物品数的范围都可以由 $2^i$ 组合得到，转化为 0-1 背包问题
        -   同样可以使用一维空间优化
...
-   定义
    -   状态表示某一个区间
-   例题：[石子合并]([282. 石子合并 - AcWing题库](https://www.acwing.com/problem/content/284/))
    -   状态 $dp[i][j]$
        -   将所有第 i 堆石子到第 j 堆石子合并成一堆石子的合并方式

    -   状态计算
        -   类似快排，pivot 从 $[i + 1, j - 1]$ 中取 min
